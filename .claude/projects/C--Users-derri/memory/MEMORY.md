...
- **Mobile Project**: ONLY work on `C:\Users\derri\projects\resume-ai-app\mobile` - DO NOT use `talor-web/mobile`

# Recent Completed Work
- **AI-Powered Steps Insights (Mar 2, 2026)** - Integrated GPT-4.1-mini AI insights into steps page. StepsContext now generates personalized insights using OpenAI based on: steps data (today/weekly/trend/streak), walk data (total/distance/averages), calorie data (consumed/remaining/goal from MealPlanContext + GoalWizardContext), personal records (best day/longest walk), and user's first name. Added aiInsights state, isGeneratingInsights loading, 2-second debounce to prevent excessive API calls. Smart fallback to static rule-based insights if AI fails. AIInsightsCard displays AI-generated content with type/title/body structure. Context integration via useMealPlan() and useGoalWizard() hooks. Commit: 6e4b00b
- **Nike/Apple-Style Pace Heatmap (Mar 3, 2026)** - Phase 1 MVP implementation. Changed from 4-bucket to 6-bucket Nike-style pace visualization (very_fast→fast→moderate→slow→very_slow→rest) with green→yellow→orange→red gradient. Added rest stop detection with >10s threshold (auto-pause tracking with location, timestamps, duration). New RestStop interface and Walk fields (restStops[], activeTimeSeconds). Enhanced gpsTrackingService with getRestStops()/getTotalRestTime() methods. Created reusable PaceLegend component (liquid glass, top-right overlay, all 6 buckets). Updated WalkMapView (live map legend), WalkDetailModal (replaced incomplete legend, added rest stop stat, "Active Time" label), RouteHistoryCard (rest count indicator "• X stops"). Backward compatibility in walkStorageService (defaults for old walks). Filters traffic lights <10s. Commit: d4496ad
- **Wearable Integrations - Fitbit/Garmin/Whoop/Withings (Mar 2, 2026)** - Full OAuth 2.0 connect/callback/sync/disconnect for 4 new providers. Fitbit: PKCE flow, sleep stages, SpO2, HRV, breathing rate, steps (150 req/hr). Garmin: Health API, sleep score/stages, SpO2, respiration, steps. Whoop: sleep stages in ms, recovery endpoint for HRV/SpO2/HR (100 req/min). Withings: POST-based API, sleep summary, stages, HR, breathing rate (120 req/min). Generic getProviderTokens() + refreshProviderToken() helpers. Multi-provider sync router replaces Oura-only. Env vars needed: FITBIT_CLIENT_ID/SECRET, GARMIN_CLIENT_ID/SECRET, WHOOP_CLIENT_ID/SECRET, WITHINGS_CLIENT_ID/SECRET. DB: code_verifier on oauth_states, external_user_id on connected_devices. Commit: dd3113d
- **Sleep Tracking Feature (Mar 2, 2026)** - Comprehensive sleep analytics: Apple Health sleep stage integration, SleepScoreRing, SleepStageBar, SleepTrendChart, SleepDebtGauge, BreathingExercise, SleepInsightCard, SleepConsistencyGauge, VitalsGrid components. New app/(tabs)/sleep.tsx page. Enhanced SleepRecoveryCard. Dashboard compact cards (Sleep Score, Total Sleep, Deep+REM). Backend sleep analytics endpoint. Commit: 3fcc66b
- **Steps Intelligence Platform (Mar 2, 2026)** - [Full details](steps-intelligence-platform.md). Major rewrite transforming basic steps page into GPS walking intelligence platform. 30+ new files across 5 sprints. Key: StepsContext (single source of truth replacing duplicate HealthKit calls), WalkContext (GPS state machine: idle→active→paused→summarizing), gpsTrackingService (Haversine, pace, auto-pause, RDP decimation), 16 section components, WalkTrackerSheet (full-screen map tracker), 3 new DB tables (walks, step_records, step_streaks), 6 new API endpoints. Requires `npx expo run:ios` rebuild for react-native-maps + expo-keep-awake.
- **Scalable Backend Architecture (Feb 27, 2026)** - Major backend overhaul implementing AWS Well-Architected patterns. Added Redis+BullMQ async job queue (AI endpoints return 202+jobId, worker process handles generation). ExternalServiceGateway with circuit breaker, retry, timeout, concurrency semaphore for all OpenAI calls. Pino structured JSON logging. Prometheus metrics (/metrics). Fixed health check to return 503 when DB down. Per-user Redis rate limiting with free/pro tiers. Zod input validation. Sentry error tracking. Explicit DB pool config (max:20, timeouts). Client-side job polling in aiService.ts. New files: 10 backend modules. Railway needs: Redis service + worker service. Env vars: REDIS_URL, SENTRY_DSN. Commit: 78fbbe0
- **iOS Resume Upload Auth Fix (Feb 23, 2026)** - Fixed 401 authentication error on iOS resume uploads in resume-ai-app. Root cause: expo-file-system/legacy has known bugs on iOS SDK 50+ where headers are silently dropped. Solution: Replaced uploadResume() to use direct fetch + FormData with explicit auth headers (Authorization + X-User-ID). Removed uploadResumeFile() method using FileSystem.uploadAsync. Updated UploadResumeScreen and CareerPathDesignerScreen to use FormData approach. Commit: 3bc240f
- **Subscription & Paywall Implementation (Feb 23, 2026)** - Integrated full subscription system with feature gating. Created SubscriptionContext.tsx (mock mode for dev without RevenueCat), PaywallModal.tsx (liquid glass design, $14.99/mo or $99.99/yr). Added feature gating to MealPlanContext: checks canGenerateMealPlan() before AI generation, returns "PAYWALL_REQUIRED" error when limit exceeded, tracks usage after successful generation. Free users get 1 lifetime meal plan, Pro users get 2/week. Backend usage_limits table with weekly/monthly auto-reset. Pricing: $14.99/month or $99.99/year (44% savings). Status: 4/6 enhancements complete. Commits: b323e03, d5c32b9
- **AI Coach Reset & Avatar-First Greetings (Feb 23, 2026)** - Added reset chat button to all AI coach modals. Implemented avatar-first loading: avatar loads and becomes ready, then generates personalized AI greeting using GPT-4.1-mini (contextual by time of day, recognizes user by name), avatar speaks greeting, then chat interface loads. Added generateCoachGreeting() to aiService.ts with fallback logic. Added /api/v1/ai/coach-greeting backend endpoint. CoachChatModal and AccountabilityCoachModal both implement new flow. Commit: 817a0d5
- **Meal Plan Calendar - Sunday-Saturday Week (Feb 23, 2026)** - Fixed calendar to show Sunday-Saturday week format. Calendar displays: Sun 22, Mon 23, Tue 24, Wed 25, Thu 26, Fri 27, Sat 28. Uses today.getDay() to auto-select current day (0=Sun, 1=Mon, etc.). Automatic midnight reset updates dates and selection. Commits: ba80425, b85bb1d, 6b25d38
- **Calendar Card Width & Horizontal Macros (Feb 23, 2026)** - Increased day card width from 72px to 92px for better macro visibility. Changed macros from 3-column grid to horizontal row with dividers (P | C | F). Single glass card matching calories layout. Updated fonts, padding, scroll calculation. Commits: 51bc39e, 844493a
