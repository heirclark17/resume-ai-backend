...
- **Meal Plan Calendar - Start from Today (Feb 23, 2026)** - Fixed calendar to start from TODAY's date instead of Sunday of week. Changed updateDatesToCurrentWeek() and generateAIMealPlan() to use today as day 0. Set selectedDayIndex to 0 (first day = today). Calendar now shows: Mon Feb 23 (today), Tue Feb 24, Wed Feb 25, etc. Automatic midnight reset updates dates. Addresses "always starts on the local date of the user real time". Commits: ba80425, b85bb1d
- **Calendar Card Width Increase (Feb 23, 2026)** - Increased day card width from 72px to 92px for better macro visibility. Increased padding, font sizes (label 6→7, value 9→10), and spacing. Updated scroll calculation. Macro numbers now clearly readable. Commit: 51bc39e
